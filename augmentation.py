    ...
    def readImageGenerator(self, rastr_dir, mask_dir, get_random):
        ...
        if get_random:
            try:
                ...
                rastr = ImageProcessing.read(f"{rastr_files[count]}.{formats}")
                mask = ImageProcessing.read(f"{masks_files[count]}.{formats}")
            ...
            for count, _ in enumerate(ids):
                try:
                    rastr = ImageProcessing.read(
                        f"{rastr_files[count]}.{formats}")
                    mask = ImageProcessing.read(
                        f"{masks_files[count]}.{formats}")
                    concatenated = self.concatenateImage(rastr, mask)
                    rastr = None
                    mask = None
                    yield concatenated
    ...
    for count, ci in tqdm(enumerate(concatenated_images)):
        ...
        if augmented_images != []:
            for aug_num, (image, mask) in enumerate(augmented_images):
                ...
                toSaveMask = os.path.join(mask_path, f"{basename}_augment_{aug_num}.{formats_mask}")
                toSaveImage = os.path.join(image_path, f"{basename}_augment_{aug_num}.{formats_image}")
